import type { Band, ScoreBreakdown, ScoreContribution, SignalResult } from "../types";
...
export function aggregateSignals(signals: SignalResult[]): AggregatedScore {
  ...
  for (const signal of signals) {
    ...
    const weightedNumerator = weight * risk * confidence;
    const weightedDenominator = weight * confidence;
  ...
  const riskScore = hardFlag ? Math.max(initialScore, 75) : Math.min(initialScore, 65);
  ...
  return {
    ...
    hardFlag,
    breakdown: {
      formula: "score01 = sum(weight * risk * confidence) / sum(weight * confidence)",
      contributions,
      numerator,
      denominator,
      score01,
      initialRiskScore: initialScore,
      hardFlagApplied: Boolean(hardFlag),
      hardFlagReason: hardFlag,
      calibrationNote:
        "Demo weights are hand-calibrated. High is reserved for hard signals; behaviour-only risk is capped at Elevated and would be tuned with historical reviewer outcomes in production.",
    },
  };
}
