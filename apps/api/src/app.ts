...
app.post("/api/reviewer/login", (req, res) => {
  const requestedEmail = typeof req.body?.email === "string" ? req.body.email.trim().toLowerCase() : "";
  const email = requestedEmail.includes("@") ? requestedEmail : "reviewer@carousell.demo";
...
async function scoreVerdicts(onProgress?: VerdictProgressHandler): Promise<ApiClaimVerdict[]> {
  ...
  let completed = 0;
  const concurrency = Number.parseInt(process.env.SCORE_CONCURRENCY ?? "3", 10);

  await mapWithConcurrency(claims, Number.isFinite(concurrency) ? Math.max(1, concurrency) : 3, async (claim, index) => {
