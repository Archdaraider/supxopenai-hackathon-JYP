...
async function main() {
  ...
  for (const claim of claims) {
    ...
    const expectedBand = claim._dev?.expected_band;
    const pass = !isBand(expectedBand) || scoredClaim.band === expectedBand;
    if (pass) passed += 1;
    if (claim._dev?.ground_truth === "legitimate" && scoredClaim.band === "Low") lowLegitimateKeptLow += 1;
    if (["C010", "C011", "C012"].includes(claim.id) && scoredClaim.band === "Low") logisticsKeptLow += 1;
    if (scoredClaim.band === "High") actualHigh += 1;
    if (scoredClaim.band === "High" && scoredClaim.hardFlag) highHardFlags += 1;
  ...
  console.log("note=full pipeline over available signals; unavailable OpenAI calls are dropped by design.");
