...
const DEMO_REVIEWER_EMAIL = "reviewer@carousell.demo";
        ...
        <header className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">Carousell reviewer dashboard</p>
            </div>
          </div>
        </header>
          ...
          <section className="login-hero__content grid min-h-[calc(100svh-112px)] w-full gap-8 px-4 py-8 sm:px-8 lg:grid-cols-[minmax(0,1fr)_minmax(360px,460px)] lg:items-center lg:px-10">
            <div className="max-w-3xl">
              ...
              <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-200 sm:text-lg">
                A Carousell reviewer workspace for refund-claim risk scoring, evidence review, and advisory actions.
              </p>
            </div>
              ...
              <form className="login-card p-5 sm:p-6" onSubmit={submitLogin}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-500">Carousell Review Ops</p>
                  </div>
                </div>
    ...
    <header className="flex flex-col gap-3 border-b border-zinc-800/80 pb-4 md:flex-row md:items-center md:justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500">Carousell reviewer queue</p>
        </div>
      </div>
    ...
    <section className="min-w-0 rounded-lg border border-zinc-800 bg-zinc-950/80">
      <div className="grid gap-5 border-b border-zinc-800 p-4 lg:grid-cols-[minmax(0,1fr)_280px] xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="min-w-0">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {(verdict.claim.marketplaceContext ?? ["Demo claim context"]).map((item) => (
              <span
                key={item}
                className="rounded-md border border-zinc-800 bg-zinc-950 px-2.5 py-1.5 text-xs font-medium text-zinc-400"
              >
                {item}
              </span>
            ))}
      ...
      <div className="grid gap-5 p-4 2xl:grid-cols-[minmax(0,0.95fr)_minmax(520px,1.05fr)]">
        ...
        <div className="min-w-0 space-y-5">
          ...
          <ScoreMathPanel verdict={verdict} />
...
function ScoreMathPanel({ verdict }: { verdict: ClaimVerdictView }) {
    ...
    <section className="rounded-lg border border-zinc-800 bg-ink-900 p-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-500">Score math, hard-flag rule, and calibration note.</p>
        </div>
        <span className="rounded-md border border-zinc-800 bg-zinc-950 px-2.5 py-1.5 text-xs font-medium text-zinc-400">
          High requires hard signal
        </span>
      </div>

      {!breakdown ? (
        <div className="mt-4 rounded-lg border border-dashed border-zinc-800 bg-zinc-950 px-4 py-6 text-sm text-zinc-500">
          Score breakdown was not returned by the API.
        </div>
      ) : (
        <>
            ...
            <table className="w-full min-w-[560px] border-collapse text-left text-xs">
              <thead className="bg-zinc-950 text-zinc-500">
                <tr>
                  ...
                  <th className="px-3 py-2 font-medium">Numerator</th>
                  <th className="px-3 py-2 font-medium">Denominator</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-800">
                {breakdown.contributions.map((row) => (
                  <tr key={row.signal} className="text-zinc-300">
                    <td className="px-3 py-2 font-medium text-white">{formatSignalLabel(row.signal)}</td>
                    <td className="px-3 py-2">{formatPercent(row.risk)}</td>
                    <td className="px-3 py-2">{formatPercent(row.confidence)}</td>
                    <td className="px-3 py-2">{row.weight.toFixed(1)}</td>
                    <td className="px-3 py-2">{row.weightedNumerator.toFixed(3)}</td>
                    <td className="px-3 py-2">{row.weightedDenominator.toFixed(3)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-4">
            <MathStat label="Numerator" value={breakdown.numerator.toFixed(3)} />
            <MathStat label="Denominator" value={breakdown.denominator.toFixed(3)} />
            <MathStat label="Initial score" value={String(breakdown.initialRiskScore)} />
            <MathStat
              label="Hard flag"
              value={breakdown.hardFlagApplied ? breakdown.hardFlagReason ?? "Applied" : "None"}
            />
          </div>

          <p className="mt-3 rounded-lg border border-zinc-800 bg-zinc-950 p-3 text-xs leading-5 text-zinc-400">
            {breakdown.calibrationNote}
          </p>
        </>
      )}
    </section>
...
function SignalDetails({ signal }: { signal: SignalView }) {
  ...
  if (signal.name === "VisualClaimIntegrity") {
    const observedDamage = asStringList(details.observedDamageFeatures);
    const expectedFailures = asStringList(details.expectedFailureModes);
    ...
    return (
      <div className="border-t border-zinc-800 p-3">
        ...
        />
        <DetailSection title="Observed damage" lines={observedDamage.length ? observedDamage : ["Not returned."]} />
        <DetailSection title="Expected material failures" lines={expectedFailures.length ? expectedFailures : ["Not returned."]} />
  ...
  if (signal.name === "ImageReuse") {
    const matched: Record<string, unknown> = isRecord(details.matched)
      ? details.matched
      : isRecord(details.closestMatch)
        ? details.closestMatch
        : {};
    const thresholds = isRecord(details.thresholds) ? details.thresholds : {};
      ...
      <div className="border-t border-zinc-800 p-3">
        <DetailGrid
          items={[
            ...
            ["pHash distance", asNumber(details.minDistance)],
            ["Match type", asString(matched.kind) || "None"],
            ["Match source", asString(matched.claimId) || asString(matched.productId) || "None"],
          ]}
        />
        ...
        <DetailSection
          title="Thresholds"
          lines={[
            `Hard flag: ${asString(thresholds.hardFlagDistance) || "<5"}`,
            `Elevated: ${asString(thresholds.elevatedDistance) || "<=8"}`,
          ]}
        />
      </div>
    );
...
function formatSignalLabel(name: string): string {
  if (name === "VisualClaimIntegrity" || name === "ImageReuse" || name === "BehaviouralContext") {
    return formatSignalName(name);
  }
