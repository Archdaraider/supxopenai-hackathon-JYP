...
Hackathon demo for refund-claim integrity triage in a Carousell marketplace.
A Carousell internal reviewer signs in, opens a queue of buyer refund claims, and
...
1. A demo Carousell reviewer signs in to the dashboard.
...
claim input form, metadata/EXIF signal, or real Carousell integration.
...
Without a hard flag, the score is capped at `Elevated`; behaviour alone cannot force a `High` band.
...
`/api/seller/login` remains only as a legacy compatibility alias. The reviewer workflow uses
`/api/reviewer/login`.
...
`eval:deterministic` does not call OpenAI. `eval:pipeline` can run without OpenAI by dropping
unavailable model signals and scoring over the available deterministic signals. `eval:signal1`
requires OpenAI environment variables. The make-or-break regression gate is that behaviour-only
frauds stay `Elevated`, legitimate false-positive anchors stay `Low`, and the
`C010`/`C011`/`C012` logistics cluster remains `Low`.

See `PROOF.md` for the complete judge-facing verification trail.
...
- Repository workflow: `main` -> `staging` -> `feature`.
