...
A Carousell internal reviewer claim-integrity dashboard for **cash refund claims**, with
physical-plausibility reasoning as its core differentiator. A Carousell reviewer logs in, sees buyer
cash refund claims (damage photos → payout request), and gets a **0–100 Risk Score** plus a band (Low / Elevated / High), per-signal
evidence, and a plain-language explanation. It is advisory triage for the reviewer, not an
auto-reject. Designed to counter the rising scam of AI-generated fake damage photos submitted to
claim monetary refunds on goods received intact — reasoning about the *whole claim*, not just pixel forensics.

**Why cash refunds specifically:** fraudsters who submit fake damage photos want the cash payout
while keeping the item. They have no incentive to return a product they actually received intact — so
return-based fraud is self-limiting. Cash refund fraud (especially partial refunds, which require no
return at all under Carousell policy) is where the abuse concentrates.

---

## The Problem

**Cash refund fraud via fake damage photos** has moved from anecdote to industry-scale threat.
The attack is simple: buyer receives an intact item, photographs or generates a fake damage image,
submits a refund claim citing damage, and pockets both the item and the refund payout.

Under Carousell's policy, a full refund requires returning the item — but fraudsters have no
intention of returning intact goods. Instead they target **partial refunds** (no return required
under policy) or push for full refund payouts before the return condition is enforced.
...
- PYMNTS documented (March 2026) a wave of shoppers submitting AI-generated images of damaged
  products to claim cash refunds on goods received intact.
...
Carousell's public dispute process is manual human investigation with a ~3-day review SLA. There is no
public AI-native adjudication of damage-photo claims. Incumbent tooling (Riskified, Ravelin) is
Shopify/Amazon-centric; SEA marketplaces are underserved. That's the gap: AI-native triage of
**cash refund claims with photo evidence**, as a transparent layer at the front of the existing pipeline.

---

## What We're Building / NOT Building

**Building:** a Carousell internal reviewer dashboard backed by a claim-integrity agent. The reviewer
...
- ❌ Real Carousell integration. The product story assumes Carousell webhook/database intake; demo JSON stands in for it.
...
- **Data:** `data/products.json`, `data/accounts.json`, `data/orders.json`, `data/claims.json`;
  ...
  refund claims delivered by Carousell platform data, not reviewer-entered form input.
...
1. (10s) **Hook/login:** Carousell reviewer logs in and lands on the dashboard. "Buyers are submitting AI-generated damage photos to claim cash refunds on items they actually received intact. They keep the item and pocket the payout. We use AI to catch it before it pays out."
...
Takeaway slide: "What takes ~3 days to investigate, our agent triages in ~5 seconds — for a Carousell reviewer to verify."
...
- **Incumbent shadow.** Carousell likely has internal fraud ML. Our novelty is the physical-plausibility
...
> We're building a real-time refund-claim integrity system for Carousell. Refund abuse via AI-generated
...
> Carousell's current public process is manual investigation with a ~3-day review SLA; there is no
...
> see their own claims; only Carousell sees across the platform — and only Carousell can detect a buyer
...
> I built a dropship detection tool for buyers on Carousell and Carousell in Southeast Asia. You paste a
