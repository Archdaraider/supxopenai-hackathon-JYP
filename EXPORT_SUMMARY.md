...
- In: demo reviewer login, Carousell internal claim dashboard, 0–100 **Risk Score** + band (Low/Elevated/High) +
...
- Out: no DB/ORM, no production auth, no EXIF/metadata/C2PA signal, no extra signals,
  no production deploy, no buyer UI, no manual claim input form, no real Carousell integration.
